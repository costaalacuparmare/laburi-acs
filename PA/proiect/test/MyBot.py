...
while True:
    ...
    moves = []
    game_map.get_frame()
    border = deque([Task(initial_potential(square), initial_potential(square), 0, square) for square in game_map if square.owner != myID])
    pf_map = dict()
    ...
    while len(pf_map) < game_map.width * game_map.height and border:
        ...
        if square not in pf_map:
            pf_map[square] = (1 - alpha) * task.initial_potential + alpha * potential_degradation_step * task.distance ** 2
