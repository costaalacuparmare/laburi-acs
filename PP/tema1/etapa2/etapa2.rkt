...
(define (ast-func suffixes)
  (let* ((new-suffixes (foldr (lambda (x acc)
                                (if (null? x)
                                    (cons '() acc)
                                    (cons (cdr x) acc)))
                              '() suffixes))
         (ast-pair (cons (list (car (car suffixes))) new-suffixes))
         )
    (if (null? (cadr ast-pair))
        (list (car ast-pair))
        ast-pair
        )
    )
...
(define (cst-func suffixes)
  (let* ((common-prefix (longest-common-prefix-of-list suffixes))
         (cst-pair (reverse (foldl (lambda (x acc)
                                     (cons (cadr (longest-common-prefix x common-prefix)) acc))
                                   (list common-prefix)
                                   suffixes))))
    (if (null? (cadr cst-pair))
        (list (car cst-pair))
        cst-pair
        )
    )
  ...
  (define lose-nulls
    (lambda (pair)
      (filter (lambda (element)
                (not (null? (cadr element)))) pair))
    )
  (define group-suffixes
    (lambda (letter)
      (list (list letter) (filter (lambda (suffix)
                                    (equal? letter (car suffix))) suffixes)))
    )

  (let* ((first-letter-match-pairs-list (map group-suffixes alphabet))
         (labeled-branches (map (lambda (pair)
                                  (labeling-func (cadr pair))) (lose-nulls first-letter-match-pairs-list))))
    (map (lambda (branch)
           (if (null? (cdr branch))
               branch
               (cons (car branch) (suffixes->st labeling-func (cdr branch) alphabet)))) labeled-branches)
    )
  )
  ...
  (lambda (text)
    (lambda (labeling-func)
      (let* ((suffixes (get-suffixes (append text '(#\$))))
        ...
        (suffixes->st labeling-func suffixes sorted-alphabet))
      )
    )
  )
