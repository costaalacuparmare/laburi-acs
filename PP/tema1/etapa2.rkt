...
(define (get-ch-words words ch)
    (filter (lambda (x) (and (not (null? x)) (equal? (car x) ch))) words)
)
...
(define (cst-func suffixes)
    (let* ((prefix (longest-common-prefix-of-list suffixes)))
        (if (null? prefix)
            '()
            (cons prefix (map (lambda (x) (drop x (length prefix))) suffixes))
        )
    )
)
...
(define (suffixes->st labeling-func suffixes alphabet)
  (if (or (null? suffixes) (null? alphabet))
      '()
      (let* ((ch (car alphabet))
             (ch-words (get-ch-words suffixes ch))
             (new-suffixes (map cdr ch-words))
             (labeling (labeling-func ch-words)))
        ;(displayln ch)
        ;(displayln ch-words)
        ;(displayln new-suffixes)
        ;(displayln labeling)
        (cons labeling (suffixes->st labeling-func suffixes alphabet))))
)
    (define (f n res)
        (if (= n 6)
            res
...
(suffixes->st ast-func suff-1 (string->list "abn$"))
    ; Helper function to update suffixes for a group
    ;  (define (update-suffixes acc ch suffix)
    ;    (map (lambda (pair)
    ;           (if (equal? (car pair) ch)
    ;               (cons ch (cons suffix (cdr pair)))
    ;               pair))
    ;         acc))
    ;  ; Step 1: Get all suffixes of the input text
    ;  (define all-suffixes (get-suffixes suffixes))
    ;  ; Step 2: Group suffixes by their first character
    ;  (define grouped-suffixes
    ;    (foldl (lambda (suffix acc)
    ;             (let ((ch (car suffix)))
    ;               (if (assoc ch acc)
    ;                   (update-suffixes acc ch suffix)
    ;                   (cons (cons ch (list suffix)) acc))))
    ;           '() all-suffixes))
    ;
    ;  ; Step 3: Calculate label and new suffixes for each group
    ;  (define labeled-suffixes
    ;    (map (lambda (group)
    ;           (let ((ch (car group))
    ;                 (suffixes (cdr group)))
    ;             (cons (car (labeling-func suffixes))
    ;                   (map cdr suffixes))))
    ;         grouped-suffixes))
    ;
    ;  ; Step 4: Recursively build subtrees for each group
    ;  (map (lambda (labeled-suffix)
    ;         (cons (car labeled-suffix)
    ;               (if (empty? (cdr labeled-suffix))
    ;                   '()
    ;                   (suffixes->st labeling-func (cdr labeled-suffix) alphabet))))
    ;       labeled-suffixes)
