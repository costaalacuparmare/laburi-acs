...
removeDuplicatesLeft l =
    reverse (foldl (\acc x -> if (elem x acc) then acc else (x : acc)) [] l)
...
removeDuplicatesRight l =
    foldr (\x acc -> if (elem x acc) then acc else (x : acc)) [] l
...
computeLength getLineSegment getStartPoint getEndPoint =
    let {
        segment = getLineSegment; start = getStartPoint segment;
        stop = getEndPoint segment; x1 = fst start; y1 = snd start;
        x2 = fst stop; y2 = snd stop;
      }
...
playerWins2 candies =
  player candies
      where player candies = (candies > 0) && ((not (opponent (candies - 1))) || (not (opponent (candies - 2))))
            opponent candies = (candies > 1) && ((not (player (candies - 2))) || (not (player (candies - 3))))
