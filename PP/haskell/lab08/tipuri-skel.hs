...
data BST a = BSTNil | BST (BST a) a (BST a) deriving Show

insertElem :: (Ord a, Eq a) => BST a -> a -> BST a
insertElem BSTNil x = BST BSTNil x BSTNil
insertElem (BST left val right) x =
  if x < val
    then BST (insertElem left x) val right
    else if x > val
      then BST left val (insertElem right x)
      else BST left val right

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil _ = Nothing
findElem (BST left val right) x =
  if x < val
    then findElem left x
    else if x > val
      then findElem right x
      else Just val

inorder :: BST a -> [a]
inorder BSTNil = []
inorder (BST left val right) = inorder left ++ [val] ++ inorder right

size :: BST a -> Int
size BSTNil = 0
size (BST left _ right) = 1 + size left + size right
...
isBalanced (BST left _ right) = abs (height left - height right) <= 1 &&
