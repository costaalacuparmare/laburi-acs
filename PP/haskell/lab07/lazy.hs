...
setUnion :: Eq a => [a] -> [a] -> [a]
setUnion a b = a ++ [x | x <- b, x `notElem` a]
...
newtonRaphsonSolve x_old atol g dg = result
  where
    ...
    vals = zip (tail nr) nr
    result = fst $ head $ dropWhile (\(x, y) -> abs (x - y) > atol) vals
...
babylonianMethod a x_old atol = newtonRaphsonSolve x_old atol (\x -> x ** 2 - a) (\x -> x * 2)
