...
printTab :: Show a => Char -> Int -> BST a -> [Char]
printTab _ _ BSTNil = ""
printTab tab level (BSTNod elem left right) = replicate level tab ++ show elem ++ "\n"
    ++ printTab tab (level + 1) left
    ++ printTab tab (level + 1) right

instance Show a => Show (BST a) where
    show BSTNil = ""
    show tree = printTab '\t' 0 tree
...
instance Invertible (BST a) where
    ...
    invert (BSTNod elem left right) = BSTNod elem (invert right) (invert left)
...
instance Functor BST where
    ...
    fmap f (BSTNod elem left right) = BSTNod (f elem) (fmap f left) (fmap f right)
...
instance Foldable BST where
    ...
    foldr f acc (BSTNod elem left right) = foldr f (f elem (foldr f acc right)) left
...
sizeFold :: BST a -> Int
sizeFold BSTNil = 0
sizeFold tree = foldr (\x y -> y + 1) 0 tree
