...
instance Show RegionAST where
    show region = showRegion 0 region
      where
        ...
        showRegion level (R region) = case region of
            FromPoints points -> replicate (2 * level) ' ' ++ "FromPoints " ++ show points
            Rectangle width height -> replicate (2 * level) ' ' ++ "Rectangle " ++ show width ++ " " ++ show height
            Circle radius -> replicate (2 * level) ' ' ++ "Circle " ++ show radius
            Complement region' -> replicate (2 * level) ' ' ++ "~\n" ++ showRegion (level + 1) region'
            Union region1 region2 -> replicate (2 * level) ' ' ++ "+\n" ++ showRegion (level + 1) region1 ++ "\n" ++ showRegion (level + 1) region2
            Intersection region1 region2 -> replicate (2 * level) ' ' ++ "*\n" ++ showRegion (level + 1) region1 ++ "\n" ++ showRegion (level + 1) region2
            Transform transformation region' -> replicate (2 * level) ' ' ++ show transformation ++ "\n" ++ showRegion (level + 1) region'
...
basicEntityCount = foldRegionAST combiner
  where
    ...
    combiner region = case region of
        ...
        Rectangle _ _ -> (1, 0)
        Circle _ -> (1, 0)
        Complement (reg1, trans1) -> (reg1, trans1)
        Union (reg1, trans1) (reg2, trans2) -> (reg1 + reg2, trans1 + trans2)
        Intersection (reg1, trans1) (reg2, trans2) -> (reg1 + reg2, trans1 + trans2)
        Transform transformation (reg, trans) -> (reg, trans + basicTransformationCount transformation)
