...
plot width height region =
      ...
      where
          line :: Int -> String
          line y = [if region (fromIntegral x, fromIntegral y)
                    then '*'
                    else '.' | x <- [-width .. width]]
...
{-
    ...
    Exemple: Atunci cand utilizam evaluarea lenesa, punctele din regiunea
    reprezentata de cercuri sunt calculate doar atunci cand este necesar,
    adica atunci cand dorim sa verificam apartenenta unui punct la acea regiune
...
{-
    ...
    Răspuns: Atunci cand verificam apartenenta unui punct care nu apartine
    regiunii infinite, evaluarea recursiva continua va genera cercuri
    la infinit. Aceasta reuniune infinita va crea o succesiune nesfarsita de
    cercuri, fiecare translatata la dreapta cu 6 unitati fata de cea
    anterioara, fara sa ajunga vreodata la un rezultat concret.
    Evaluarea va continua la infinit sau va conduce la blocarea procesului,
    din cauza recursivitatii fara a avea o condiție de oprire.
-}
