...
void run_chat_multi_server(int listenfd) {
  ...
  poll_fds[num_sockets].fd = timerfd;
      ...
      if (poll_fds[i].revents & POLLIN) {
        if (poll_fds[i].fd == listenfd) {
          ...
          printf("Noua conexiune de la %s, port %d, socket client %d\n",
            ...
            struct chat_packet commercial_add;
            memcpy(commercial_add.message, "Promotie la toate produsele!\n",
                   sizeof("Promotie la toate produsele!\n"));
            commercial_add.len = sizeof("Promotie la toate produsele!\n");

            for (int j = 1; j < num_sockets; j++) {
                if (poll_fds[j].fd != timerfd) {
                    rc = send_all(poll_fds[j].fd, &commercial_add,
                                  sizeof(commercial_add));
                    if (rc <= 0) {
                        perror("send_all");
                        return;
                    }
