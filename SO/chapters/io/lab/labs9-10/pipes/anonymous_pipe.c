...
static void child_loop(int readfd)
{
    ...
    while (1)
    {
        ...
        if (rc == 0)
        {
            /* Close pipe head used for reading. */
            close(readfd);
            break;
        }
...
static void parent_loop(int writefd)
{
    ...
    while (1)
    {
        ...
        {
            /* Close pipe head used for writing. */
            close(writefd);
            break;
        }
...
int main(void)
{
    ...
    switch (pid)
    {
        ...
        case 0: /* Child process. */
            /* Close unused pipe head by child. */
            close(pipedes[PIPE_WRITE]);

            /* Call child loop and pass pipe head used for reading. */
            child_loop(pipedes[PIPE_READ]);

            break;

        default: /* Parent process. */
            /* Close unused pipe head by parent. */
            close(pipedes[PIPE_READ]);

            /* Call parent loop and pass pipe head used for writing. */
