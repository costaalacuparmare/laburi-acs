...
int main(void)
{
    ...
    /* Add address and port to receiver_addr. */
    /* HINT: use populate_sockaddr(). */
    populate_sockaddr(&receiver_addr, localhost, PORT);

    /* Bind sockfd to receiver_addr. Use DIE to check for errors. */
    ...
    while (1) {
        ...
        /* Receive at most BUFSIZE bytes through the socket with recvfrom(). */
